...
let busyCount = 0;
function ensureBusyLayer(){ let el = document.getElementById('globalBusy'); if(!el){ el = document.createElement('div'); el.id='globalBusy'; el.innerHTML='<div class="busy-card"><div class="busy-bar"><span></span></div><b>Processing...</b><p>Please wait. Action is being saved.</p></div>'; document.body.appendChild(el); } return el; }
...
function itemCard(i){ const cat=state.categories.find(c=>c.id===i.categoryId)?.name||''; return `<button class="item-card clickable-card ${i.soldOut?'sold':''}" data-add-item="${esc(i.id)}" ${i.soldOut?'disabled':''}>${i.soldOut?'<div class="sold-badge">SOLD OUT</div>':''}<div class="item-img">${i.imageUrl?`<img src="${esc(i.imageUrl)}" alt="">`:'<span class="image-placeholder">No image</span>'}</div><div class="item-body"><div><h4>${esc(i.name)}</h4><p>${esc(cat)}</p><div class="price">${money(i.price)}</div></div></div></button>`; }
function renderDeals(){
  $('#centerScroll').innerHTML = `<div class="card subcard"><div class="module-title"><h3>Deals</h3>${can('admin.menu')?'<button class="ghost-btn" onclick="screen=\'admin\';adminTab=\'deals\';renderShell()">Manage Deals</button>':''}</div><div class="item-grid">${state.deals.filter(d=>d.active).sort((a,b)=>(a.sort||0)-(b.sort||0)).map(d=>`<button class="item-card clickable-card deal-card" data-add-deal="${esc(d.id)}"><div class="popular">Deal</div><div class="item-img">${d.imageUrl?`<img src="${esc(d.imageUrl)}" alt="">`:'<span class="image-placeholder">No image</span>'}</div><div class="item-body"><div><h4>${esc(d.name)}</h4><p>${esc(d.description||'Deal')}</p><div class="price">${money(d.price)}</div></div></div></button>`).join('') || '<div class="empty-cart">No active deals. Add real deals from Admin → Deals.</div>'}</div></div>`;
...
async function saveOrder(hold=false){ if(!currentOrder) return; if(!requireOrderLines(hold?'Add at least one item before Hold':'Add at least one item before Save')) return; try{ const j=await api('/api/orders/save',{...currentOrder,hold}); currentOrder=hold?null:j.order; await loadState(); renderShell(); toast(hold?'Order held':'Order saved'); }catch(e){toast(e.message,true);} }
