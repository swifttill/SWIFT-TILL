...
function renderSidebar(){
  ...
  <div class="sidebar-scroll">
    ...
    <button class="cat-btn ${centerMode==='deals'?'active':''}" id="dealsBtn"><span>Special Deals</span></button>
    <button class="cat-btn" id="comboBtn"><span>Meal Combos</span></button>
  </div>`;
}
...
function uploadFile(file, folder='uploads'){ return new Promise((resolve,reject)=>{ const r=new FileReader(); r.onload=async()=>{try{const j=await api('/api/upload-image',{filename:file.name,dataUrl:r.result,folder});resolve(j.url)}catch(e){reject(e)}}; r.onerror=()=>reject(new Error('File read failed')); r.readAsDataURL(file); }); }
