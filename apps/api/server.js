...
const APP_VERSION = '58.0.0-pin-registration-stability';
...
process.on('unhandledRejection', (err) => {
  console.error('Unhandled rejection protected:', err && err.stack ? err.stack : err);
});
process.on('uncaughtException', (err) => {
  console.error('Uncaught exception protected:', err && err.stack ? err.stack : err);
});
...
function defaultPinActive(user) { return [...DEFAULT_PINS].some(pin => verifyUserPin(user, pin)); }
function pinSet(user) { return Boolean(user?.pinHash || user?.pin); }
...
function sanitizeUserForClient(u = {}) { const defaultPasswordActive = verifyPassword('admin123', u.passwordHash || u.password || '') || verifyPassword('manager123', u.passwordHash || u.password || '') || verifyPassword('cashier123', u.passwordHash || u.password || ''); const defaultPin = defaultPinActive(u); return { id: u.id, name: u.name, email: u.email, roleIds: u.roleIds || [], active: u.active !== false, passwordSet: Boolean(u.passwordHash || u.password), pinSet: pinSet(u), defaultPinActive: defaultPin, pinSetupRequired: defaultPin, defaultPasswordActive }; }
...
function publicUser(db, u) { const permissions = userPermissions(db, u); return { id: u.id, name: u.name, email: u.email, roleIds: u.roleIds || [], roles: (u.roleIds || []).map(id => db.roles.find(r => r.id === id)?.name).filter(Boolean), permissions, pinSet: pinSet(u), defaultPinActive: defaultPinActive(u), pinSetupRequired: defaultPinActive(u) }; }
...
async function handleApi(req, res, pathname, query) {
  try {
    ...
    if (pathname === '/api/account/register-pin' && req.method === 'POST') {
      const b = await parseBody(req);
      const u = db.users.find(x => x.id === user.id);
      if (!u) throw Object.assign(new Error('User not found'), { status: 404 });
      if (!b.password || !verifyUserPassword(u, b.password)) throw Object.assign(new Error('Current password is incorrect'), { status: 403 });
      const pin = assertPinFormat(b.pin);
      if (String(b.confirmPin || '') !== pin) throw Object.assign(new Error('PIN confirmation does not match'), { status: 422 });
      const hadPin = pinSet(u);
      const hadDefaultPin = defaultPinActive(u);
      if (hadPin && !hadDefaultPin) throw Object.assign(new Error('PIN is already registered. Use Change PIN.'), { status: 409 });
      if (isDefaultPinValue(pin)) throw Object.assign(new Error('Do not use the default PIN. Choose a new PIN.'), { status: 422 });
      setUserPin(u, pin);
      db.pinEvents = Array.isArray(db.pinEvents) ? db.pinEvents : [];
      db.pinEvents.unshift({ id: uid('pin'), action: hadDefaultPin ? 'DEFAULT_PIN_REPLACED' : 'PIN_REGISTERED', targetUserId: u.id, byUserId: u.id, by: u.name, at: now() });
      db.pinEvents = db.pinEvents.slice(0, 500);
      audit(db, user, hadDefaultPin ? 'DEFAULT_PIN_REPLACED' : 'PIN_REGISTERED', { userId: u.id, email: u.email });
      await saveDb(db, hadDefaultPin ? 'default-pin-replaced' : 'pin-registered');
      return send(res, 200, { ok: true, message: hadDefaultPin ? 'Default PIN replaced' : 'PIN registered', user: sanitizeUserForClient(u) });
    }
    if (pathname === '/api/account/change-pin' && req.method === 'POST') { const b = await parseBody(req); const u = db.users.find(x => x.id === user.id); if (!u) throw Object.assign(new Error('User not found'), { status: 404 }); if (!(u.pinHash || u.pin)) throw Object.assign(new Error('No PIN exists. Register PIN first.'), { status: 409 }); const currentOk = (b.currentPin && verifyUserPin(u, b.currentPin)) || (b.password && verifyUserPassword(u, b.password)); if (!currentOk) throw Object.assign(new Error('Current PIN or password is incorrect'), { status: 403 }); const pin = assertPinFormat(b.newPin || b.pin, 'New PIN'); if (String(b.confirmPin || '') !== pin) throw Object.assign(new Error('PIN confirmation does not match'), { status: 422 }); if (isDefaultPinValue(pin)) throw Object.assign(new Error('Do not use the default PIN. Choose a new PIN.'), { status: 422 }); setUserPin(u, pin); db.pinEvents = Array.isArray(db.pinEvents) ? db.pinEvents : []; db.pinEvents.unshift({ id: uid('pin'), action: 'PIN_CHANGED', targetUserId: u.id, byUserId: u.id, by: u.name, at: now() }); db.pinEvents = db.pinEvents.slice(0, 500); audit(db, user, 'PIN_CHANGED', { userId: u.id, email: u.email }); await saveDb(db, 'pin-changed'); return send(res, 200, { ok: true, user: sanitizeUserForClient(u) }); }
    if (pathname === '/api/account/remove-pin' && req.method === 'POST') { const b = await parseBody(req); const u = db.users.find(x => x.id === user.id); if (!u) throw Object.assign(new Error('User not found'), { status: 404 }); if (!b.password || !verifyUserPassword(u, b.password)) throw Object.assign(new Error('Current password is incorrect'), { status: 403 }); clearUserPin(u); db.pinEvents = Array.isArray(db.pinEvents) ? db.pinEvents : []; db.pinEvents.unshift({ id: uid('pin'), action: 'PIN_REMOVED', targetUserId: u.id, byUserId: u.id, by: u.name, at: now() }); db.pinEvents = db.pinEvents.slice(0, 500); audit(db, user, 'PIN_REMOVED', { userId: u.id, email: u.email }); await saveDb(db, 'pin-removed'); return send(res, 200, { ok: true, user: sanitizeUserForClient(u) }); }
